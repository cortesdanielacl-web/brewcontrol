...
import { RecipePackagingFormatSection } from './RecipePackagingFormatSection';
import { RecipePackagingCostsSection } from './RecipePackagingCostsSection';
...
export const RecipeStage3EvaluacionComercial: React.FC<RecipeStage3EvaluacionComercialProps> = ({
  recipe,
  currency,
  onUpdateRecipe,
  onBack,
  ...
  const packagingType = getPackagingType(normalizePackagingFormatMl(recipe.bottleSizeMl));
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      {savedToast && (
        <div className="bg-[#0D1B2A] text-white px-5 py-3 rounded-2xl bc-shadow border border-[#F5A623] flex items-center gap-3 animate-in slide-in-from-top-2 duration-200">
          ...
          <div>
            <p className="text-xs font-bold">Evaluación comercial guardada</p>
            <p className="text-[11px] text-slate-300">Costos de envasado registrados en tu receta</p>
          </div>
        </div>
      )}

      <RecipeReadOnlySummarySection recipe={recipe} currency={currency} showCostPerLiter />
      <RecipePackagingFormatSection
        recipe={recipe}
        currency={currency}
        onFormatChange={handleFormatChange}
      />
      {!isKeg && (
        <RecipePackagingCostsSection
          recipe={recipe}
          currency={currency}
